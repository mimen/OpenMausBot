...
export function GeneralRow({
  ...
  return (
    <div className="grid grid-cols-[4px_minmax(0,1fr)] gap-3 py-2.5 sm:grid-cols-[4px_minmax(0,1fr)_auto] sm:items-start">
      <span className={cn("h-full min-h-8 rounded-full", ROW_TONES[tone])} aria-hidden="true" />
      <div className="min-w-0">
        <div className="flex min-w-0 items-start gap-2">
          {leading ? <span className="mt-0.5 shrink-0 text-ink-secondary">{leading}</span> : null}
          <div className="min-w-0">
            <div className={cn("text-[13.5px] font-medium leading-5 text-ink", completed && "text-ink-secondary line-through")}>{title}</div>
...
const CHIP_STYLES = {
  existing: "bg-raised text-ink-secondary",
...
export function WeekStrip({ days }: { days: WeekStripDay[] }) {
  return (
    <div className="-mx-1 overflow-x-auto px-1 pb-1" aria-label="Seven day calendar">
      <div className="grid min-w-[700px] grid-cols-7 gap-2 sm:min-w-0">
        {days.map((day) => (
          <section key={day.date} className="min-w-0 rounded-xl bg-inset/70 p-2" aria-label={`${day.label}, ${day.date}`}>
            ...
            <div className="grid gap-1.5">
              {day.fieldTrips?.map((trip) => (
                <div key={trip.id} className="rounded-md bg-warning/15 px-1.5 py-1 text-[10.5px] leading-4 text-warning">
                  <CalendarDays size={10} className="me-1 inline" aria-hidden="true" />{trip.label}{trip.time ? ` · ${trip.time}` : ""}
                </div>
              ))}
              {day.chips.map((chip) => (
                <div key={chip.id} className={cn("rounded-md px-1.5 py-1 text-[10.5px] leading-4", CHIP_STYLES[chip.kind])}>
                  {chip.time ? <span className="me-1 tabular-nums">{chip.time}</span> : null}{chip.label}
                </div>
              ))}
              {day.chips.length === 0 && !day.fieldTrips?.length ? <span className="py-2 text-center text-[10.5px] text-ink-secondary">Open</span> : null}
            </div>
          </section>
        ))}
      </div>
...
export function StatusIcon({ tone }: { tone: RowTone }) {
  ...
  if (tone === "critical" || tone === "serious" || tone === "warning") return <CircleAlert size={14} className={tone === "critical" ? "text-danger" : "text-warning"} aria-hidden="true" />;
